#!/usr/bin/env python3
"""
修复 Jittor depthwise_conv.py 中的 jt.code 调用问题 - 版本2
正确使用 jt.code 的多输出语法
"""
...
def fix_depthwise_conv_grad(depthwise_conv_path):
    ...
    # 查找需要修复的代码段 - 找到 jt.code 调用的开始
    code_start = content.find('return jt.code([x.shape, weight.shape], [x.dtype, weight.dtype], [x, weight, grad],')
    if code_start == -1:
    ...
    # 创建修复后的调用
    # 根据 jt.code 文档，对于多个输出应该这样调用：
    # jt.code([shape1, shape2], [dtype1, dtype2], inputs, ...)
...
def main():
    print("=== Jittor DepthwiseConv 修复工具 v2 ===")

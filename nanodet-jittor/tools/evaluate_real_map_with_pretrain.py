...
def decode_predictions_improved(outputs, strides=[8, 16, 32, 64], reg_max=7, num_classes=20, conf_threshold=0.3):
    """改进的预测解码"""
    ...
    for b in range(batch_size):
        ...
        for a in range(num_anchors):
            ...
            if max_score.item() > conf_threshold:  # 提高置信度阈值
                # 解码bbox
                anchor_x, anchor_y = anchor_points[a]
                dx, dy, dw, dh = bbox_preds[b, a]
                
                # 改进的bbox解码
                x1 = anchor_x.item() + dx.item() * 100 - dw.item() * 50
                y1 = anchor_y.item() + dy.item() * 100 - dh.item() * 50
                x2 = anchor_x.item() + dx.item() * 100 + dw.item() * 50
                y2 = anchor_y.item() + dy.item() * 100 + dh.item() * 50
...
def evaluate_model_with_pretrain():
    """评估预训练模型"""
    logger = logging.getLogger(__name__)
    logger.info("=" * 50)
    logger.info("预训练模型真实mAP评估")
    logger.info("=" * 50)
    ...
    try:
        ...
        for img_info in images[:100]:  # 评估前100张图像
            ...
            # 解码预测
            detections = decode_predictions_improved(outputs, conf_threshold=0.3)
        ...
        if len(aps) > 0:
            mean_ap = np.mean(aps)
            logger.info(f"\n🎯 预训练模型 mAP = {mean_ap:.4f}")
            
            if mean_ap > 0.05:  # 预训练模型应该有更好的性能
                logger.info("✅ 预训练模型mAP > 0.05，检测性能良好")
                return True
            else:
                logger.warning(f"⚠️ 预训练模型mAP较低: {mean_ap:.4f}")
...
def main():
    ...
    logger.info("开始预训练模型真实mAP评估...")
    
    success = evaluate_model_with_pretrain()
    
    if success:
        logger.info("🎉 预训练模型真实mAP评估完成！")
        logger.info("✅ NanoDet Jittor版本预训练模型验证成功！")
        return True
    else:
        logger.error("❌ 预训练模型真实mAP评估失败！")
        return False

#!/usr/bin/env python3
"""
修复 Jittor depthwise_conv.py 中的 jt.code 调用问题 - 版本3
使用正确的 jt.code_ 函数
"""
...
def fix_depthwise_conv_grad(depthwise_conv_path):
    ...
    # 查找需要修复的代码段 - 找到修复后的 jt.code 调用
    code_start = content.find('input_grad, weight_grad = jt.code([x.shape, weight.shape], [x.dtype, weight.dtype], [x, weight, grad],')
    if code_start == -1:
        print("❌ 未找到需要修复的 jt.code 调用")
        return False
    ...
    # 创建修复后的调用
    # 根据 jt.code 文档，对于多个输出应该使用 jt.code_：
    # jt.code_([shape1, shape2], [dtype1, dtype2], inputs, ...)
    fixed_call = f"""input_grad, weight_grad = jt.code_([x.shape, weight.shape], [x.dtype, weight.dtype], [x, weight, grad],
        {cuda_content}"""
...
def main():
    print("=== Jittor DepthwiseConv 修复工具 v3 ===")

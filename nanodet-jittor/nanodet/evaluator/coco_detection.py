...
class CocoDetectionEvaluator:
    """COCO Detection Evaluator for Jittor NanoDet.
    
    Args:
        dataset: Dataset instance with coco_api.
        save_key (str): Key to save best model. Default: 'mAP'.
    """
    
    def __init__(self, dataset, save_key="mAP"):
        assert hasattr(dataset, "coco_api"), "Dataset must have coco_api attribute"
    ...
    def results2json(self, results):
        ...
        Args:
            results (dict): {image_id: {label: [bboxes...] } }
            
        Returns:
            list: COCO json format results.
        """
        json_results = []
    ...
    def evaluate(self, results, save_dir, rank=-1):
        ...
        # Print per class AP - 完全模仿PyTorch版本的表格格式

...
class GhostModule(nn.Module):
    ...
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, activation='ReLU'):
        ...
        self.cheap_operation = nn.Sequential(nn.Conv(init_channels, new_channels, dw_size, stride=1, padding=(dw_size // 2), groups=init_channels, bias=False), nn.BatchNorm(new_channels), (act_layers(activation) if activation else nn.Sequential()))
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, dw_kernel_size=3, stride=1, activation='ReLU', se_ratio=0.0):
        ...
        if ((in_chs == out_chs) and (self.stride == 1)):
            ...
            self.shortcut = nn.Sequential(nn.Conv(in_chs, in_chs, dw_kernel_size, stride=stride, padding=((dw_kernel_size - 1) // 2), groups=in_chs, bias=False), nn.BatchNorm(in_chs), nn.Conv(in_chs, out_chs, 1, stride=1, padding=0, bias=False), nn.BatchNorm(out_chs))
